...
export function XpRankWidget() {
    ...
    if (loading) return <div className="text-center text-slate-500"> Loading Rank...</div>
        ...
        <div className="w-full">
            <div className="rounded-lg border-purple-600 border-2 mt-4 p-4 space-y-3 hover:shadow-lg hover:shadow-purple-400 transition duration-400 ease-in-out">
                ...
                <div className="w-full h-2 bg-slate-200 rounded-full">
                    <div className={`h-2 rounded-full ${progress.percent === 100 ? "bg-amber-500" : "bg-blue-500"}`} style={{ width: `${progress.percent}%` }} />
                </div>
            </div>

            <Link href="/dashboard/xp" className="block mt-4 text-right text-blue-500 hover:text-amber-50 duration-500 ease-in-out"> View All {`->`}</Link>
        </div>
    )
}
